...
void neepan()
{
  ...
  fr(i, 0, n - 1)
  {
    ...
    if (sum < 0 || (i != 0 && abs(a[i]) % 2 == abs(a[i - 1]) % 2))
    {
      // since we are looping fromt the 0th index and we are using the a[i-1] that why we are doing i!=0, which will segmentation fault
      sum = 0;
    }
