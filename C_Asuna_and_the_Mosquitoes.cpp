...
void neepan()
{
    ...
    // same parity max()
    //  else sum-(x-1) --> x is the no of odd values. We will be left over (x-1)no. of 1's after the operation

    vi arr(n);

    lli c0 = 0, c1 = 0;
    // c0 -> no. of even values || c1--> no. of  odd values

    fr(i, 0, n - 1)
    {
        cin >> arr[i];
        if (arr[i] % 2) // if a[i] is odd
            c1++;
        else
            c0++;
    }
    ...
    lli ans = accumulate(all(arr), 0LL) - (c1 - 1);
    cout << ans << endl;
}
